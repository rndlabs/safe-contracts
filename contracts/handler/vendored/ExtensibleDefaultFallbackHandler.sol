...
import "../ExtensibleHandlerContext.sol";
import "../../Safe.sol";

/**
 * @title Compatibility Fallback Handler - Provides compatibility between pre 1.3.0 and 1.3.0+ Safe contracts.
 * @dev This instance of the Compatibility Fallback Handler is designed to be used as the default fallback handler
 *      for `ExtensibleFallbackHandler`. This is the same as the `CompatibilityFallbackHandler` but with the
 *      vendoring for:
 *      - `msg.sender` replaced with `_manager()`
...
contract ExtensibleDefaultFallbackHandler is TokenCallbackHandler, ISignatureValidator, ExtensibleHandlerContext {
    ...
    function getMessageHash(bytes memory message) public view returns (bytes32) {
        return getMessageHashForSafe(Safe(payable(_manager())), message);
    }
    ...
    function isValidSignature(bytes32 _dataHash, bytes calldata _signature) external view returns (bytes4) {
        ISignatureValidator validator = ISignatureValidator(_manager());
