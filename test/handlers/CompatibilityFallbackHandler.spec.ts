...
import {
    ...
    extensibleDefaultFallbackHandlerContract,
    getCompatFallbackHandler,
    getExtensibleDefaultFallbackHandler,
...
const suite = [
    {
        ...
        deployment: getCompatFallbackHandler,
        contract: compatFallbackHandlerContract,
    },
    {
        name: "ExtensibleDefaultFallbackHandler",
        deployment: getExtensibleDefaultFallbackHandler,
        contract: extensibleDefaultFallbackHandlerContract,
    },
];
    ...
    describe(test.name, async () => {
        ...
        const setupTests = deployments.createFixture(async ({ deployments }) => {
            ...
            const handlerAddress = (
                await (test.name === "ExtensibleDefaultFallbackHandler" ? getExtensibleFallbackHandler() : getCompatFallbackHandler())
            ).address;

            const signLib = await (await hre.ethers.getContractFactory("SignMessageLib")).deploy();
            const handler = await test.deployment();
            const signerSafe = await getSafeWithOwners([user1.address], 1, handlerAddress);
            const safe = await getSafeWithOwners([user1.address, user2.address, signerSafe.address], 2, handlerAddress);
            const validator = (await test.contract()).attach(safe.address);
        ...
        describe("ERC1155", async () => {
            ...
            it("to handle onERC1155BatchReceived", async () => {
                const { handler } = await setupTests();
                await expect(await handler.callStatic.onERC1155BatchReceived(AddressZero, AddressZero, [], [], "0x")).to.be.eq(
                    "0xbc197c81",
                );
            });
        });
            ...
            it("should return magic value if enough owners signed and allow a mix different signature types", async () => {
                ...
                expect(
                    await validator.callStatic["isValidSignature(bytes,bytes)"](
                        "0xbaddad",
                        buildSignatureBytes([sig1, sig2, signerSafeSig]),
                    ),
                ).to.be.eq("0x20c13b0b");
            });
        });
        ...
        describe("getMessageHashForSafe", async () => {
            it("should revert if target does not return domain separator", async () => {
                const { handler, handlerAddress } = await setupTests();
        ...
        describe("simulate", async () => {
            ...
            it("should return result", async () => {
                const { validator, killLib, handlerAddress } = await setupTests();
